...
// a metric
double distance(spherical_point p1, spherical_point p2) {
  cartesian_point c1 = cartesian(p1);
  cartesian_point c2 = cartesian(p2);
...
// connect two nodes
void connect_nodes( node* nodes, int node1, int node2 ) {
  for (int i=0; i<6; i++) {
...
graph subdivide( graph g, int edges ) {
  ...
  for (int n1=0; n1<g.size; n1++) {
	...
	connect_nodes(result.nodes, n1, midpoints[6*n1+i]);
	for (int j=0; j<6; j++) {
	  ...
	  if ( connected(g.nodes, g.nodes[n1].neighbors[i], g.nodes[n1].neighbors[j]) ) {
	    connect_nodes(result.nodes, midpoints[6*n1+i], midpoints[6*n1+j]);
	  }
	}
      }
    }
...
graph icosphere(int subdivisions) {
  ...
  for (int i=0; i<10; i+=2) {
    connect_nodes(ico.nodes, 10, i);
  }
  
  // bottom connects to the odd nodes
  for (int i=1; i<10; i+=2) {
    connect_nodes(ico.nodes, 11, i);
  }
  ...
  for (int i=0; i<10; i++) {
    ...
    connect_nodes(ico.nodes, i, a);
    connect_nodes(ico.nodes, i, b);
    connect_nodes(ico.nodes, i, c);
    connect_nodes(ico.nodes, i, d);
  }
